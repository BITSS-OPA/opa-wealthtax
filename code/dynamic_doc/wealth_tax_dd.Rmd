...
The final `r paste0(evasion_param_in * 100,"%")` tax avoidance/evasion response to a `r paste0(main_tax_po * 100, "%")` wealth tax was computed as an average across these four studies (`r paste0(main_tax_po * 100, "%", "*(", ela1_so, "+", ela2_so, "+" ,(ela3_1_so+ela3_2_so)/2, "+", (ela4_1_so+ela4_2_so)/2, ")/4")`).
...
The `r target_hhlds_mo` households with assets totaling over `r paste0("$",starting_brack_po/1e6)` million would have a total taxable wealth (above the `r paste0("$",starting_brack_po/1e6, "million")`) of `r paste("$", round(tax_base_total_mo, digits = 1), sep="")` trillion,  i.e. approximately `r paste0(round(tax_base_total_mo/total_wealth_in* 100), "%")`  of the `r paste0("$", round(total_wealth_in))` trillion population-wide, total household net worth.

#### 4.2 - `r paste0((max_tax_po - main_tax_po) * 100, "%")` additional tax on all wealth above `r paste0("$",next_increase_po/1e9)` billion

The `r target_hhlds_bn_mo` households with assets totaling over `r paste0("$",next_increase_po/1e9)` billion dollars would have a total taxable wealth (above the `r paste0("$",next_increase_po/1e9, "billion")`) of `r paste("$", round(tax_base_total_surtax_mo, digits = 1), sep="")` trillion,  i.e. approximately `r paste0(round(tax_base_total_surtax_mo/total_wealth_in* 100), "%")`  of the `r paste0("$", round(total_wealth_in))` trillion population-wide, total household net worth.


### 5 - Total tax revenue in one year

```{r new-tax-rev, eval=TRUE, echo=print_code, warning = FALSE} 
# DESCRIBE FUNCTIONS STRUCTURE
# - inputs: list
# - outputs: list
#### function:  
...
```
 ...
 <font size="4"> Tax revenue from wealth tax over 10 years: `r paste0("$", round(ten_year_revenue_pe/1e3, 1))` trillion</font>
...
[^3]: With r=`r paste0(100 * discount_rate_mo, "%")`, we have [1+(1+r)+..+(1+r)^9]=[(1+r)^10-1]/r=`r round(sum((1 + discount_rate_mo)^(0:9)),1)`, or approximately 13.
