...
# Elasticities
ela1 <- as.numeric(research_df[1,"param"])           # 0.5 - David. 2017
ela2 <- as.numeric(research_df[2,"param"])           # 0.5 - Jakobsen et al. 2018
ela3_1 <- as.numeric(research_df[3,"param"])         # 2 - Londono-Velez 2018
ela3_2 <-   as.numeric(research_df[4,"param"])       # 3 - Londono-Velez 2018
ela4_1 <- as.numeric(research_df[5,"param"])         # 23 - Brülhart et al. 2016
ela4_2 <- as.numeric(research_df[6,"param"])         # 34 - Brülhart et al. 2016
...
We write regarding your proposal to impose a progressive annual wealth tax on American households with net worth (sum of all assets net of debts) above `r paste("$", tax_starts_at/1e6, "m", sep = "")` million. The tax would be 2% on the net worth above \$50 million with an additional 1% tax on net worth above \$1 billion. We estimate that about `r  format(round( sum( numberTaxpayers[brackets>=starting_brack] )/1000 ) * 1000, scientific=FALSE)` American households (less than  `r paste(round(sum( numberTaxpayers[brackets>=starting_brack] )/total_hhlds * 100 , digits = 1), "%", sep = "")`) would be liable for the wealth tax and that the tax would raise around **`r paste("$", round(ten_year_revenue/1e3, 2) - adj_factor1, sep = "")` trillion** over the ten-year budget window 2019-2028, of which `r paste("$", round(ten_year_top_tax/1e3,1), sep ="")` trillion would come from the billionaire 1% surtax. The wealth tax would raise approximately 1.0% of GDP per year (\$210 billion relative to a \$21.1 trillion GDP in 2019).
