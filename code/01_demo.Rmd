...
```{r tax-elasticity}
final_ela <- mean(c(ela1, ela2, (ela3_1 + ela3_2)/2, (ela4_1 + ela4_2)/2))
evasion_param <- 0.02 * final_ela - adj_factor1
```
...
#This is the taxed money (the aboves is the totale taxable) in billions 
...
target_hhlds_round <- format(round( sum( numberTaxpayers[brackets>=50e6] )/1000 ) * 1000, scientific=FALSE)
...
[^4]: With r=5.5%, we have [1+(1+r)+..+(1+r)^9]=[(1+r)^10-1]/r=`r sum((1 + 0.055)^(0:9))`, approximately 13.
