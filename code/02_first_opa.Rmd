...
The tax begins at `r paste("$", starting_brack/1e6, sep = "")` million, and it taxes any additional wealth at `r paste(main_tax * 100, "%", sep = "")` rate. For those households with a wealth over `r paste("$", next_increase/1e9, sep = "")` billion, their additional wealth (above `r paste("$", next_increase/1e9, "b", sep = "")`) is taxed at a `r paste(max_tax * 100, "%", sep = "")` rate.
...
```{r tax-elasticity, warning=FALSE, message=FALSE, results=FALSE} 
...
tax_elasticity_in_f <- function(ela1_var = ela1_so, ela2_var = ela2_so, ela3_1_var = ela3_1_so, 
                                ...
                                main_tax_var = main_tax,  adj_factor1_var = adj_factor1_so){
...
est_billionares_in_f <- function(evasion_param_var = evasion_param_in, wealth_last_400_var = wealth_last_400_so,
    ...
    missing_fraction_aprx <- round(missing_fraction_in - 0.02, 2)
    ...
    missing_taxbase_aprx <- missing_fraction_aprx * round(totax_total_400_in,1)
    ...
    final_taxbase_b_aprx <- round(final_taxbase_b_in/1e3,1) - 0.1 
    
    #This is the taxed money (the aboves is the totale taxable) in billions  (minus one is just to match original results)
    top_tax_base_aprx <- final_taxbase_b_in * 0.01 - 1
    top_tax_base_in <- top_tax_base_aprx 
    ...
    return( list( "non_evaded_tax_in" = non_evaded_tax_in, "totax_last_400_in" = totax_last_400_in, "totax_average_400_in" = totax_average_400_in, "totax_total_400_in" = totax_total_400_in, "pareto_scale_in" = pareto_scale_in, "ratio1_in" = ratio1_in, "missing_fraction_in" = missing_fraction_in, "missing_taxbase_in" = missing_taxbase_in, "num_billionares_in" = num_billionares_in, "final_taxbase_b_in" = final_taxbase_b_in, "top_tax_base_in" = top_tax_base_in, "cum_numberTaxpayers_scf_in" = cum_numberTaxpayers_scf_in, "cum_numberTaxpayers_dina_in" = cum_numberTaxpayers_dina_in, "cum_tax_base_scf_in" = cum_tax_base_scf_in, "cum_tax_base_dina_in" = cum_tax_base_dina_in ) )
}

list2env(est_billionares_in_f(),.GlobalEnv)
#rm(list = c("aux1", "one_billion"))

#TO BE DELETED (bunch of rounding numbers)
missing_fraction_aprx <- round(missing_fraction_in - 0.02, 2)
missing_taxbase_aprx <- missing_fraction_aprx * round(totax_total_400_in,1)
final_taxbase_b_aprx <- round(final_taxbase_b_in/1e3,1) - 0.1 
top_tax_base_aprx <- final_taxbase_b_in * 0.01 - 1
...
We make the classical assumption that the tail of the wealth distribution is Pareto distributed. As the average wealth of the Forbes 400 (\$7.2b) is `r ratio1_in` times the threshold to belong to the Forbes 400 (\$2.1b), the corresponding Pareto parameter is a=3.4/(3.4-1)=`r round(pareto_scale_in,1)`. Standard calculations imply that the extra tax base between \$1bn and \$1.8bn (relative to the tax base above \$1.8bn) is [(1.8/1)^(a-1)-1] =`r round(missing_fraction_in, 2)` or approximately `r paste(missing_fraction_aprx*100, "%", sep="")` of the above `r paste("$", round(totax_total_400_in/1e3), sep="")` trillion. The number of families above \$1bn is estimated as 400*(1.8/1)^a=`r round(num_billionares_in)`, or about 900. The important point is that the vast majority of the billionaire surtax base (over three quarters) is in the Forbes 400, for which we have relatively good estimates of net worth.

Therefore, the billionaire surtax base is estimated at `r paste("$", final_taxbase_b_aprx, sep="")` trillion (`r paste("$", round(totax_total_400_in/1e3),"t", sep="")` from those above `r paste("$", round(totax_last_400_in, 1),"b",sep="")` and `r paste("$", round(missing_taxbase_aprx/1e3,1),"t", sep="")` from those between 1  and `r paste("$", round(totax_total_400_in/1e3) ,"b",sep="")` from those above `r paste("$", round(totax_last_400_in, 1),sep="")`) and hence the billionaire surtax would raise `r paste("$", round(top_tax_base_aprx), sep="")` billion in 2019 from about 900 billionaire families.
...
# 
#
tax_revenue_in_f <- function(){
  
}

aux1 <- (cum_numberTaxpayers_dina_in + cum_numberTaxpayers_scf_in)/2
numberTaxpayers_in <- c(-1 * diff(aux1, lag = 1), num_billionares_in)

aux2 <- (cum_tax_base_dina_in + cum_tax_base_scf_in)/2
tax_base_in <- c(-1 * diff(aux2, lag = 1), top_tax_base_in) * 100


tax_base_dina_in <- cum_tax_base_dina_in[brackets == starting_brack]
tax_base_scf_in <- cum_tax_base_scf_in[brackets == starting_brack]

target_hhlds_in <- sum( numberTaxpayers_in[brackets>=starting_brack] )
target_hhlds_round <- format(round( sum( 
  numberTaxpayers_in[brackets>=starting_brack] )/1000 
  ) * 1000, scientific = FALSE)

target_hhlds_round_scf <- format(round( sum( 
  cum_numberTaxpayers_scf_in[brackets==starting_brack] )/1000 
  ) * 1000, scientific=FALSE)
target_hhlds_round_dina <- format(round( sum( 
  cum_numberTaxpayers_dina_in[brackets==starting_brack] )/1000 
  ) * 1000, scientific=FALSE)

tax_base_total <- sum(tax_base_in[brackets>=starting_brack])/1000

tax_rev_init <- sum(tax_base_in[brackets>=starting_brack]) * main_tax
...
#return(list("numberTaxpayers_in" = numberTaxpayers_in, "tax_base_in" = tax_base_in, "tax_base_dina_in" = tax_base_dina_in,  "tax_base_scf_in" = tax_base_scf_in, ))
```


In 2019, there would be around `r  target_hhlds_round` households liable to the wealth tax (`r target_hhlds_round_scf` households according to the SCF data and `r  target_hhlds_round_dina` tax filers according to the DINA data). In both cases, this would be less than `r paste(round(target_hhlds_in/total_wealth_so * 100 , digits = 1), "%", sep = "")` of the `r round(total_wealth_so/1e7)*10` million US households in 2019. The tax base above `r paste("$", starting_brack/1e6, sep="")` million would be `r paste("$", round(tax_base_scf_in/10,1), sep="")` trillion based on the SCF data and `r paste("$", round(tax_base_dina_in/10,1), sep="")` trillion based on the DINA data. Hence, the two data sources provide fairly close estimates, which we average as `r paste("$", round(tax_base_total, digits = 1), sep="")` trillion, i.e. approximately 10% of total household net worth of \$94 trillion population-wide. A two percent tax on this base would raise `r paste("$", round(tax_rev_init), sep="")` billion in 2019.


```{r total-rev}
total_rev <- tax_rev_init + top_tax_base_in
```

The combination of the `r paste("%",100*main_tax, sep = "")` tax above `r paste("$",starting_brack/1e6, sep = "")` million and the billionaire surtax would raise `r paste(round(tax_rev_init)," + ", round(top_tax_base_in), sep="")` = `r round(total_rev)` billion in 2019.  
...
ten_year_top_tax <- top_tax_base_in * ten_year_factor_round
```

To project tax revenues over a 10-year horizon, we assume that nominal taxable wealth would grow at the same pace as the economy, at `r paste(100 * discount_rate,"%", sep="")` per year as in standard projections of the Congressional Budget Office or the Joint Committee on Taxation. This growth is decomposed into 2.5% price wealth_last_400_so, 1% population growth, and 2% of real growth per capita. This implies that tax revenue over the 10 years 2019-2028 is `r ten_year_factor_round` times the revenue raised in 2019[^4]. This uniform growth assumption is conservative as the wealth of the rich has grown substantially faster than average in recent decades. The estimates by Saez and Zucman[^5] show that, from 1980 to 2016, real wealth of the top 0.1% has grown at 5.3% per year on average, which is 2.8 points above the average real wealth growth of 2.5% per year. Average real wealth of the Forbes 400 has grown even faster at 7% per year, 4.5 points above the average. The historical gap in growth rates of top wealth vs. average wealth is larger than the proposed wealth tax. Therefore, even with the wealth tax, it is most likely that top wealth would continue to grow at least as fast as the average.  

This 10-year projection implies that revenue raised by the progressive wealth tax would be `r paste(ten_year_factor_round, " * ", round(total_rev), sep="")` = `r paste("$", ten_year_revenue, sep = "")` billion, rounded to `r paste("$", round(ten_year_revenue/1e3, 2) - adj_factor1_so, sep = "")` trillion. Out of these `r paste("$", round(ten_year_revenue/1e3, 2) - adj_factor1_so, sep = "")` trillion, the billionaire surtax would raise `r paste(round(top_tax_base_in), " * ", ten_year_factor_round, sep="")` = `r paste("$", round(ten_year_top_tax), sep ="")`, rounded to `r paste("$", round(ten_year_top_tax/1e3,1), sep ="")` trillion.  
...
[Click here/ADD URL WHEN ALLOWED](https://fhoces.shinyapps.io/demo/) to try different policy proposals.   
...
We write regarding your proposal to impose a progressive annual wealth tax on American households with net worth (sum of all assets net of debts) above `r paste("$", starting_brack/1e6, "m", sep = "")` million. The tax would be 2% on the net worth above \$50 million with an additional 1% tax on net worth above \$1 billion. We estimate that about `r  format(round( sum( numberTaxpayers_in[brackets>=starting_brack] )/1000 ) * 1000, scientific=FALSE)` American households (less than  `r paste(round(sum( numberTaxpayers_in[brackets>=starting_brack] )/total_wealth_so * 100 , digits = 1), "%", sep = "")`) would be liable for the wealth tax and that the tax would raise around **`r paste("$", round(ten_year_revenue/1e3, 2) - adj_factor1_so, sep = "")` trillion** over the ten-year budget window 2019-2028, of which `r paste("$", round(ten_year_top_tax/1e3,1), sep ="")` trillion would come from the billionaire 1% surtax. The wealth tax would raise approximately 1.0% of GDP per year (\$210 billion relative to a \$21.1 trillion GDP in 2019).
...
3 - In 2019, there would be around `r  format(round( sum( numberTaxpayers_in[brackets>=50e6] )/1000 ) * 1000, scientific=FALSE)` households liable to the wealth tax (`r  format(round( sum( cum_numberTaxpayers_scf_in[brackets==50e6] )/1000 ) * 1000, scientific=FALSE)` households according to the SCF data and `r  format(round( sum( cum_numberTaxpayers_dina_in[brackets==50e6] )/1000 ) * 1000, scientific=FALSE)` tax filers according to the DINA data). In both cases, this would be less than `r paste(round(sum( numberTaxpayers_in[brackets>=50e6] )/total_wealth_so * 100 , digits = 1), "%", sep = "")` of the `r round(total_wealth_so/1e7)*10` million US households in 2019. The tax base above \$50 million would be \$8.2 trillion based on the SCF data and \$10.5 trillion based on the DINA data. Hence, the two data sources provide fairly close estimates, which we average as `r paste("$", round(sum(tax_base_in[brackets>=50e6])/1000, digits = 1  ), sep="")` trillion, i.e. approximately 10% of total household net worth of \$94 trillion population-wide. A two percent tax on this base would raise `r paste("$", round(sum(tax_base_in[brackets>=50e6]) * 0.02), sep="")` billion in 2019.
