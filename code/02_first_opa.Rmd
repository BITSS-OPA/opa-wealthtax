...
### 5 - Total tax revenue (1 year) [CLEAN]
...
#199.9251
 ...
 ## gets taxes paid per bracket
  getTaxBasePerBracket <- function(grid_var = grid, taxLevels = tax_rates_po, brackets = brackets_po) {
    grid_var <- grid_var %>% filter(thresNew > starting_brack_po)
    test <- unlist(lapply(grid_var$avgNew, get_tax_rev, taxLevels, brackets))

    return(sum(grid_var$nb * test))
  }
  
  getTaxBasePerBracket()
##199.9251
...
#sapply( toPlot2$xval, function(x) get_tax_rev(wealth_var = x, taxrates_var = tax_rates_po, brackets_var = brackets_po) )
