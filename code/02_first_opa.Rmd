...
The final `r paste(evasion_param_in * 100,"%", sep="")` tax avoidance/evasion response to a `r paste0(main_tax_po * 100, "%")` wealth tax was computed as and average across these four studies (`r paste0(main_tax_po * 100, "%", "*(", ela1_so, "+", ela2_so, "+" ,(ela3_1_so+ela3_2_so)/2, "+", (ela4_1_so+ela4_2_so)/2, ")")`)
...
######
#REVIEW: Why counts from original source differ?
#    df_dina1 %>% filter(networth>50e6) %>% summarise(sum(weight))
#    grid %>% filter(thres>50e6) %>% summarise(sum(nb))
#    df_scf %>% filter(networth>50e6) %>% summarise(sum(weight))

target_hhlds_round
target_hhlds_round_scf
target_hhlds_round_dina
target_hhlds_mo
tax_base_scf_mo
tax_base_dina_mo
tax_base_total_mo
...
getTaxBasePerBracket=function(grid,brackets){
  ...
  grid$group=cut(grid$thresNew + 1, brackets)
  toReturn = grid %>% 
    group_by(group) %>% 
    summarise(taxBase = sum(avgNew * nb)) %>% 
    drop_na()
  return(toReturn)
}
