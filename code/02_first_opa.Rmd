...
tax_elasticity_in_f <- function(ela1_var = ela1_so, ela2_var = ela2_so, ela3_1_var = ela3_1_so, 
                                ...
                                main_tax_var = main_tax_po,  adj_factor1_var = adj_factor1_so){
...
tax_revenue_mo_f <- function(cum_numberTaxpayers_dina_var = cum_numberTaxpayers_dina_in, cum_numberTaxpayers_scf_var = 
    ...
    target_hhlds_round <- format(round( sum( 
      numberTaxpayers_mo[brackets_var>=starting_brack_var] )/1000 
      ) * 1000, scientific = FALSE)
    
    target_hhlds_round_scf <- format(round( sum( 
      cum_numberTaxpayers_scf_var[brackets_var==starting_brack_var] )/1000 
      ) * 1000, scientific=FALSE)
    target_hhlds_round_dina <- format(round( sum( 
      cum_numberTaxpayers_dina_var[brackets_var==starting_brack_var] )/1000 
      ) * 1000, scientific=FALSE)
    ...
    return(list("numberTaxpayers_mo" = numberTaxpayers_mo, "tax_base_mo" = tax_base_mo, "tax_base_dina_mo" =
                  ...
                  target_hhlds_mo, "tax_base_total_mo" = tax_base_total_mo,
...
ten_years_mo_f <- function(inflation_var = inflation_so, population_gr_var = population_gr_so,
    ...
    ten_year_revenue_pe <- round(total_rev_pe) * ten_year_factor_round     
    #ten_year_revenue_pe <- total_rev_pe * ten_year_factor_mo                      #PE
    ten_year_top_tax_pe <- top_tax_base_var * ten_year_factor_round          
...
if (FALSE) {
    ...
    list2env(policy_f(), .GlobalEnv)
    list2env(tax_elasticity_in_f(), .GlobalEnv)
    list2env(est_billionares_in_f(), .GlobalEnv)
    list2env(tax_revenue_mo_f(), .GlobalEnv)
    list2env(total_rev_mo_f(), .GlobalEnv)
    invisible( list2env(ten_years_mo_f(),.GlobalEnv) )
...
This 10-year projection implies that revenue raised by the progressive wealth tax would be `r paste(ten_year_factor_round, " * ", round(total_rev_pe), sep="")` = `r paste("$", round(ten_year_revenue_pe), sep = "")` billion, rounded to `r paste("$", round(ten_year_revenue_pe/1e3, 2) - adj_factor1_so, sep = "")` trillion. Out of these `r paste("$", round(ten_year_revenue_pe/1e3, 2) - adj_factor1_so, sep = "")` trillion, the billionaire surtax would raise `r paste(round(top_tax_base_in), " * ", ten_year_factor_round, sep="")` = `r paste("$", round(ten_year_top_tax_pe), sep ="")`, rounded to `r paste("$", round(ten_year_top_tax_pe/1e3,1), sep ="")` trillion.  
