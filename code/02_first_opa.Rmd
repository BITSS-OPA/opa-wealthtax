...
author:
- 'Policy Analysis: Emmanuel Saez  & Gabriel Zucman'
- 'Dynamic Document: Fernando Hoces de la Guardia'
...
###############################################################################   
    ...
    population_gr_so <- 0.01   # CBO/JCT DISCUSS WITH EMMANUEL
...
Household net worth above `r paste0("$", starting_brack_po/1e6)` million would be taxed at `r paste(main_tax_po * 100, "%", sep = "")`. Any wealth over `r paste0("$", next_increase_po/1e9)` billion would be taxed an additional 1% (a billionaire surtax).
...
knitr::kable(table_aux, format = "latex", booktabs = T) %>%
...
if(is_dina_public){
  ...
  df_dina1_so <- read_dta("analysis_data/dina.dta")
  #### End of confidential section
} 

totw_dina <- sum(df_dina1_so$networth * df_dina1_so$weight) / 1e12

### SCF data 
totw_scf <- sum(df_scf_so$networth * df_scf_so$wgt) / 1e12

#cat("TOTAL SCF NETWORTH 2016  (Tr)", totw_scf)
# Increase the population weigths to reflect population growth from 2016 to 1019
df_scf_so <- df_scf_so %>% mutate("wgt2019" = round( wgt * (1 + hhld_gr_so)^( 2019 - 2016 )))
totw <- sum(df_scf_so$networth * df_scf_so$wgt2019) / 1e12
totn <- sum(df_scf_so$wgt2019)

# Rescaling SCF to match total total wealth reported in DINA excluding the f400
df_scf1 <- df_scf_so %>% mutate("networth" = networth * ( totw_dina - f400tot ) / totw, 
                            "weight" = wgt2019, 
                            "data" = "SCF") %>%
  select(data, networth, weight)

# Combine three data sources
df <- rbind(df_forbes1, df_scf1, df_dina1_so)
...
*REPLACE AT THE END
*export delimited using "$datawork/analysis_data/tax_grid.csv", replace
```
...
#    df_dina1_so %>% filter(networth>50e6) %>% summarise(sum(weight))
...
target_hhlds_dina_mo <- df_dina1_so %>% 
  filter(networth>starting_brack_po) %>% 
  summarise(sum(weight)) %>% 
  as.numeric()
...
plot 
```
 <font size="4"> Tax revenue from wealth tax in first year: `r paste0("$", round(total_tax_rev,4))` billion </font>  
...
**[Click here/ADD APP URL](NULL) to explore different policy proposal and to see how the assumptions of the analysis affect the results.**
...
[^4]: With r=`r paste0(100 * discount_rate_mo, "%")`, we have [1+(1+r)+..+(1+r)^9]=[(1+r)^10-1]/r=`r round(sum((1 + discount_rate_mo)^(0:9)),1)`, approximately 13.
