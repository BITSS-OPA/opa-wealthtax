...
cap program drop gperc
	program gperc
		...
		quietly { 
			...
			tempvar rank 
			* Percentile rank of the variable "first"
			...
			matrix `matname' = J(`I', 5 +`nbcompo', 0)
				forval i = 1/`I' {
					...
					su `first' `wgt' if `rank'>=perc[1,`i'] & `rank'<perc[1,`i'+1] & `touse', meanonly
						...
						* Create minimum income per percentile
						* Rounding proportional to wealth level to comply with confidentiality
					...
					local j = 6
					* "rest" is descriptives in additional variables by gperc of first
					foreach c in `rest' {
